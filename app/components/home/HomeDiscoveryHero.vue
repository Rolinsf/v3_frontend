...
const { activeIndex: activeRecommendationIndex, activeItem: activeRecommendation, show: showRecommendation, previous: showPreviousRecommendation, next: showNextRecommendation, start: startCarousel, stop: stopCarousel } = useRecommendationCarousel(heroRecommendations)
...
const recentNovels = computed(() => {
  const list = [...(novels.value ?? [])].sort((a, b) => b.updatedAt.localeCompare(a.updatedAt))
  if (!auth.user) return list
  const shelfIds = new Set(bookshelf.items.map(item => item.novelId))
  const followedAuthorIds = new Set(following.authors.map(item => item.authorId))
  const personalized = list.filter(item => shelfIds.has(item.id) || followedAuthorIds.has(item.author.id))
  return personalized.length ? personalized : list
})
...
<template>
  <div>
    ...
    <HomeNovelSections
      :featured="featuredNovels"
      :recent="recentNovels"
      :debut="debutNovels"
      :categories="categories ?? []"
      :pending="novelsPending"
      :error="novelsError?.statusMessage"
      @retry="refreshNovels()"
    />
  </div>
</template>
